...
@Repository
public interface StagingPolicyMemberBankAccountRepository extends JpaRepository<StagingPolicyMemberBankAccount, Long> {
	@Query(value = "UPDATE PSTG_MEMBER_BANK_ACCOUNT m SET m.IS_ACTIVE=0 WHERE m.MEMBER_ID IN "
			+ "(SELECT MEMBER_ID FROM PSTG_MEMBER WHERE POLICY_ID=?1)", nativeQuery=true)
	void inactiveByPolicyId(Long policyId);
	
	@Modifying
	@Query(value = "DELETE FROM PSTG_MEMBER_BANK_ACCOUNT ma WHERE MEMBER_ID IN "
