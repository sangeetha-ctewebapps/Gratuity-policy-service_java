...
@Repository
public interface StagingPolicyMemberAddressRepository extends JpaRepository<StagingPolicyMemberAddressEntity, Long> {
	@Query(value = "UPDATE PSTG_MEMBER_ADDRESS m SET m.IS_ACTIVE=0 WHERE m.MEMBER_ID IN "
			+ "(SELECT MEMBER_ID FROM PSTG_MEMBER WHERE POLICY_ID=?1)", nativeQuery=true)
	void inactiveByPollicyId(Long policyId);
	
	@Modifying
	@Query(value = "DELETE FROM PSTG_MEMBER_ADDRESS WHERE MEMBER_ID IN "
