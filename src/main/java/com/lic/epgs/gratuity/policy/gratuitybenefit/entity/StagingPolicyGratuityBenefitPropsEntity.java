...
@Setter
@Entity
@Table(name = "PSTG_GRATUITY_BENEFIT_PROPS")
public class StagingPolicyGratuityBenefitPropsEntity {

	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "PSTG_GRAT_BEN_PROPS_ID_SEQ")
	@SequenceGenerator(name = "PSTG_GRAT_BEN_PROPS_ID_SEQ", sequenceName = "PSTG_GRAT_BEN_PROPS_ID_SEQ", allocationSize = 1)
	@Column(name = "GRATUITY_BENEFIT_PROPS_ID")
	private Long id;

	@ManyToOne
    @JoinColumn(name="GRATUITY_BENEFIT_ID", nullable=false)
	private StagingPolicyGratuityBenefitEntity gratuityBenefit;
