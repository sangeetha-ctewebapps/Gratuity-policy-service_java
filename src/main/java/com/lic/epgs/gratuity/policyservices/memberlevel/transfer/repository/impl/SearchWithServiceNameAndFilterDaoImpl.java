...
@Log4j2
@Repository
public class SearchWithServiceNameAndFilterDaoImpl implements UnitSearchWithServiceNameAndFilterDao {
...
//		}
	...
	public List<UnitDocumentDetailsResponse> getDocumentDetails(Long unitTransferRequisitionId) {
		log.info("Entering into getDocumentDetails : transferRequisitionId : "+unitTransferRequisitionId);
		UnitDocumentDetailsResponse documentDetailsResponse = null;

		List<UnitDocumentDetailsResponse> documentDetailsResponseList = new ArrayList<>();
		Boolean bFlag = false;
		StringBuilder sqlQuery = new StringBuilder(
		...
		if (unitTransferRequisitionId != null) {
			if (bFlag) {
				sqlQuery.append(" WHERE ");
			}
			sqlQuery.append("  TDD.TRANSFER_REQUISITION_ID  = '"
					+ unitTransferRequisitionId + "'");
			bFlag = true;
		}
		Query query = entityManager.createNativeQuery(sqlQuery.toString());
		System.out.println(sqlQuery);
		try {
			...
			if (!obj.isEmpty()) {
				for (Object[] record : obj) {
					documentDetailsResponse = new UnitDocumentDetailsResponse();
