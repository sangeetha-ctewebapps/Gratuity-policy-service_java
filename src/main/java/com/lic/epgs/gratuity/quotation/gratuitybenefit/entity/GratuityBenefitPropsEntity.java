...
@Setter
@Entity
@Table(name = "QSTG_GRATUITY_BENEFIT_PROPS")
public class GratuityBenefitPropsEntity {

	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "QSTG_GRAT_BENEF_PROPS_ID_SEQ")
	@SequenceGenerator(name = "QSTG_GRAT_BENEF_PROPS_ID_SEQ", sequenceName = "QSTG_GRAT_BENEF_PROPS_ID_SEQ", allocationSize = 1)
	@Column(name = "GRATUITY_BENEFIT_PROPS_ID")
	private Long id;

	@ManyToOne
    @JoinColumn(name="GRATUITY_BENEFIT_ID", nullable=false)
	private GratuityBenefitEntity gratuityBenefit;
